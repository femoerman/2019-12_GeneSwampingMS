...
#6) Data managing
{
  ...
  #6.8) Calculate mid log phase and K for all populations
  {
    ...
    for (i in sumdata$ident){
      ...
      if(timeK> 0){
        ...
        tempdata2 <- filter(tempdata, hours==timeInflect)
        tempdata2$logr0 <- sumdata[i, ]$logr0.mean
          dd.inf <- rbind(dd.inf, tempdata2)
      }
  ...
  dd.evo <- dd.all%>% na.omit()
  {
    sumdata.all <- rbind(sumdata.anc, sumdata.evo)
    dd.evo$percK <- NA
    for (i in 1:nrow(dd.evo)){
      K <- exp(filter(sumdata.all, ident == dd.evo[i, "curveID"])$logK.mean)
...
ggplot(filter(dd.evo, hours>0), aes(x = percK, y = gross_speed_mean, group = paste(Treatment, Gradient), colour = paste(Treatment, Gradient))) + geom_point() + geom_smooth(method = "lm") + facet_wrap(~testpH)



#7) Fit model over all data for size - evolved only
{
  full.model.speed.evo <- lme(data = dd.evo, gross_speed_mean ~ percK*testpH*Gradient*Sex +percK*testpH*Gradient*Gene.Flow +percK*testpH*Sex*Gene.Flow + percK*Gradient*Sex*Gene.Flow + testpH*Gradient*Sex*Gene.Flow, random =  list(ID=~1, curveID=~1), method = "ML")
  
  #Do manual comparison
  {
    full.model.speed.evo <- lme(data = dd.evo, gross_speed_mean ~ percK*testpH*Gradient*Sex*Gene.Flow, random =  list(ID=~1, curveID=~1), method = "ML")
    t1 <-  dredge(lme(data = dd.evo, gross_speed_mean ~ testpH*Gradient*Sex*Gene.Flow, random =  list(ID=~1, curveID=~1), method = "ML"), rank = BIC)
    t2 <-  dredge(lme(data = dd.evo, gross_speed_mean ~ percK*Gradient*Sex*Gene.Flow, random =  list(ID=~1, curveID=~1), method = "ML"), rank = BIC)
    t3 <-  dredge(lme(data = dd.evo, gross_speed_mean ~ percK*testpH*Sex*Gene.Flow, random =  list(ID=~1, curveID=~1), method = "ML"), rank = BIC)
    t4 <-  dredge(lme(data = dd.evo, gross_speed_mean ~ percK*testpH*Gradient*Sex, random =  list(ID=~1, curveID=~1), method = "ML"), rank = BIC)
    t5 <-  dredge(lme(data = dd.evo, gross_speed_mean ~ percK*testpH*Gradient*Gene.Flow, random =  list(ID=~1, curveID=~1), method = "ML"), rank = BIC)
  ...
  #best model
  best.model.speed.evo <- lme(data = dd.evo, gross_speed_mean ~ Gradient*percK + testpH*percK , random =  list(ID=~1, curveID=~1))
...
#8) Make predictions and plot + raw data
{
  ...
  ggplot(dd.evo, aes(x = percK, y = gross_speed_mean, colour = Gradient)) + geom_point(size=1) + 
    ...
    ylab("Cell speed (µm/s)") + xlab("Density (proportion of K)") +
    ...
    facet_grid(~testpH)
  ggsave(filename = "4_results/Figures/07_SpeedEvolutionAllDensities", device = "png", width = 9.53, height = 5.88, dpi = 300)
}


# ggplot(dd.inf, aes(x = logr0, y = gross_speed_mean, colour = paste(Treatment, Gradient))) + geom_point() + geom_smooth(method = "lm")+ facet_grid(testpH~.)
