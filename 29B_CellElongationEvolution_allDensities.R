...
{
  setwd("/media/felix/DataDrive2/Documenten/PhD/08_Big_Experiment_1")
...
ggplot(dd.evo, aes(x = percK, y = major_mean/minor_mean, group = Gradient, colour = Gradient)) + geom_point() + geom_smooth(method = "lm") + facet_wrap(~testpH)



#7) Fit model over all data for elongation - evolved only
{
  full.model.speed.evo <- lme(data = dd.evo, major_mean/minor_mean ~ percK*testpH*Gradient*Sex +percK*testpH*Gradient*Gene.Flow +percK*testpH*Sex*Gene.Flow + percK*Gradient*Sex*Gene.Flow + testpH*Gradient*Sex*Gene.Flow, random =  list(ID=~1, curveID=~1), method = "ML")
  
  #Do manual comparison
  {
    full.model.speed.evo <- lme(data = dd.evo, major_mean/minor_mean ~ percK*testpH*Gradient*Sex*Gene.Flow, random =  list(ID=~1, curveID=~1), method = "ML")
    t1 <-  dredge(lme(data = dd.evo, major_mean/minor_mean ~ testpH*Gradient*Sex*Gene.Flow, random =  list(ID=~1, curveID=~1), method = "ML"), rank = BIC)
    t2 <-  dredge(lme(data = dd.evo, major_mean/minor_mean ~ percK*Gradient*Sex*Gene.Flow, random =  list(ID=~1, curveID=~1), method = "ML"), rank = BIC)
    t3 <-  dredge(lme(data = dd.evo, major_mean/minor_mean ~ percK*testpH*Sex*Gene.Flow, random =  list(ID=~1, curveID=~1), method = "ML"), rank = BIC)
    t4 <-  dredge(lme(data = dd.evo, major_mean/minor_mean ~ percK*testpH*Gradient*Sex, random =  list(ID=~1, curveID=~1), method = "ML"), rank = BIC)
    t5 <-  dredge(lme(data = dd.evo, major_mean/minor_mean ~ percK*testpH*Gradient*Gene.Flow, random =  list(ID=~1, curveID=~1), method = "ML"), rank = BIC)
  ...
  #best model
  best.model.speed.evo <- lme(data = dd.evo, major_mean/minor_mean ~ Gradient*percK + percK*testpH , random =  list(ID=~1, curveID=~1))
...
#8) Make predictions and plot + raw data
{
  ...
  ggplot(filter(dd.evo, major_mean/minor_mean < 4), aes(x = percK, y = major_mean/minor_mean, colour = Gradient)) + geom_point(size=1) + 
    ...
    ylab("Cell elongation") + xlab("Density (proportion of K)") +
    ...
    facet_grid(~testpH)
  ggsave(filename = "4_results/Figures/08_ElongationEvolutionAllDensities", device = "png", width = 9.53, height = 5.88, dpi = 300)
}
