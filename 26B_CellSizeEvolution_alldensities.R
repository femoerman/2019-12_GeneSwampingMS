...
#4) read in data
{
  ...
  for (p in timepoints){
    ...
    sumdata.anc <-sumoutputall %>% filter(strain=="mix")
...
#6) Data managing
{
  ...
  dd.all$Gradient <- factor(ifelse(dd.all$Gradient=="y", "No gradient", "Gradient"), levels = c("No gradient", "Gradient"))
  ...
  sumdata.anc$logr0.mean <- sumdata.anc$logr0.mean
  sumdata.anc$logalpha.mean <- sumdata.anc$logalpha.mean
  sumdata.anc$logK.mean <- sumdata.anc$logK.mean
  sumdata.anc$logd.mean <- sumdata.anc$logd.mean
...
ggplot(dd.evo, aes(x = percK, y = major_mean, group = Gradient, colour = Gradient)) + geom_point() + geom_smooth(method = "lm") + facet_wrap(~testpH)
#dd.evo <- filter(dd.evo, testpH %in% c(4, 6.5))
#dd.evo$testpHfact <- as.factor(dd.evo$testpH)


#7) Fit model over all data for size - evolved only
{
  full.model.speed.evo <- lme(data = dd.evo, major_mean ~ percK*testpH*Gradient*Sex +percK*testpH*Gradient*Gene.Flow +percK*testpH*Sex*Gene.Flow + percK*Gradient*Sex*Gene.Flow + testpH*Gradient*Sex*Gene.Flow, random =  list(ID=~1, curveID=~1), method = "ML")
  
  #Do manual comparison
  {
    full.model.speed.evo <- lme(data = dd.evo, major_mean ~ percK*testpH*Gradient*Sex*Gene.Flow, random =  list(ID=~1, curveID=~1), method = "ML")
    t1 <-  dredge(lme(data = dd.evo, major_mean ~ testpH*Gradient*Sex*Gene.Flow, random =  list(ID=~1, curveID=~1), method = "ML"), rank = BIC)
    t2 <-  dredge(lme(data = dd.evo, major_mean ~ percK*Gradient*Sex*Gene.Flow, random =  list(ID=~1, curveID=~1), method = "ML"), rank = BIC)
    t3 <-  dredge(lme(data = dd.evo, major_mean ~ percK*testpH*Sex*Gene.Flow, random =  list(ID=~1, curveID=~1), method = "ML"), rank = BIC)
    t4 <-  dredge(lme(data = dd.evo, major_mean ~ percK*testpH*Gradient*Sex, random =  list(ID=~1, curveID=~1), method = "ML"), rank = BIC)
    t5 <-  dredge(lme(data = dd.evo, major_mean ~ percK*testpH*Gradient*Gene.Flow, random =  list(ID=~1, curveID=~1), method = "ML"), rank = BIC)
  ...
  #best model
  best.model.speed.evo <- lme(data = dd.evo, major_mean ~ testpH*percK , random =  list(ID=~1, curveID=~1))
...
#8) Make predictions and plot + raw data
{
  ...
  ggplot(filter(dd.evo, major_mean<70), aes(x = percK, y = major_mean, colour = Gradient)) + geom_point(size=1) + 
    ...
    ylab("Cell size (µm)") + xlab("Density (proportion of K)") +
    ...
    facet_grid(~testpH)
  ggsave(filename = "4_results/Figures/10_CellSizeEvolutionAllDensities", device = "png", width = 9.53, height = 5.88, dpi = 300)
}
